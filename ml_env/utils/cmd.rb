...
module Cmd
  ...
  def rsync(target_dir:, local_dir:)
    ...
    rsync_status_ok = exe "rsync --no-whole-file -r --inplace --delete --progress -e 'ssh -T -c aes128-gcm@openssh.com -o Compression=no -x ' --rsync-path=\"sudo rsync\" --exclude=\".git\" #{local_dir} #{host}:#{target_dir}"
    rsync_status_ok
  end
